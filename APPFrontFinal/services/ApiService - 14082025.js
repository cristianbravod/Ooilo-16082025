...
// services/ApiService.js - SECCIÓN ACTUALIZADA
		...
		async getPlatosEspeciales() {
		  try {
			...
			if (Array.isArray(response)) {
			  // ✅ Doble filtro de seguridad: solo vigentes
			  const platosVigentes = response.filter(p => 
				p.vigente === true || p.vigente === 1 || p.vigente === "true"
			  );
			  
			  console.log(`✅ Platos especiales: ${response.length} total, ${platosVigentes.length} vigentes`);
			  
			  // Guardar en cache solo los vigentes
			  await this.guardarEnCache(platosVigentes);
			  return platosVigentes;
			}
			...
			try {
			  ...
			  if (cached) {
				...
				// Filtrar vigentes del cache también
				return parsed.filter(p => p.vigente === true);
			  }
			} catch (cacheError) {
		...
		async createPlatoEspecial(platoData) {
		  try {
			...
			console.log('✅ Plato especial creado:', response);
			return response;
		  } catch (error) {
		...
		async updatePlatoEspecial(id, platoData) {
		  try {
			...
			console.log('✅ Plato especial actualizado:', response);
			return response;
		  } catch (error) {
	...
	async deletePlatoEspecial(id) {
	  try {
		console.log(`🗑️ Eliminando plato especial ${id} (borrado lógico)`);
		
		const response = await this.request(`/platos-especiales/${id}`, {
		  method: 'DELETE'
		});
		
		console.log('✅ Plato especial eliminado (vigente = false)');
		return response;
	  } catch (error) {
